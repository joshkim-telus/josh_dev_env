...
-- CREATE OR REPLACE PROCEDURE {dataset_id}.sp_persist_{stack_name}(_from_dt DATE, _to_dt DATE, _date_trunc_day_week_month STRING)
...
SET _label_1_sql = 
  ...
  SELECT CASE WHEN UPPER('<date_trunc_type>') = 'DAY'
         ...
         target_ind AS label, 
         _ref_dt AS label_dt,
    FROM stage_{stack_name}_label1
    ;
  """;
...
SET _label_0_sql =
  ...
  SELECT CASE WHEN UPPER('<date_trunc_type>') = 'DAY'
         ...
         target_ind AS label, 
         _ref_dt AS label_dt,
    FROM stage_{stack_name}_label0 
   WHERE target_ind = 0;
  """;
...
calc_ntile AS (
SELECT ref_dt,
       NTILE(6) OVER(ORDER BY ref_dt DESC) AS tile --change this to 10 if you want 2 months test & val each
...
# Insert into table
####################################################################################################
DELETE FROM {dataset_id}.master_target_table
...
INSERT INTO {dataset_id}.master_target_table
(
  from_dt,
  to_dt,
  training_mode,
