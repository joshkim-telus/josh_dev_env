CREATE OR REPLACE TABLE FUNCTION `divg-groovyhoon-pr-d2eab4.shs_churn.app_usg_cust_tf`(_from_dt DATE, _to_dt DATE) AS 
...
SELECT base.ref_dt,
  ...
  LEFT JOIN `bi-srv-features-pr-ef5a93.app_usage.bq_ban_imsi_app_cat_usg_daily` AS app_usg
   ...
   AND app_usg.event_dt BETWEEN DATE_SUB(base.ref_dt, INTERVAL 8 WEEK) AND DATE_SUB(base.ref_dt, INTERVAL 1 DAY)
   AND app_usg.event_dt BETWEEN DATE_SUB(_from_dt, INTERVAL 8 WEEK) AND DATE_SUB(_to_dt, INTERVAL 1 DAY)
 GROUP BY 1, 2, 3
)
SELECT *
  FROM cte_app_usg
