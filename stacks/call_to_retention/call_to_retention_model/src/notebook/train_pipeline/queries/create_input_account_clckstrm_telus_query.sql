...
stage_identity AS (
  SELECT DISTINCT 
   ...
   WHERE DATE(cust_idnty_billg_acct_ts) BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 91 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date)
 ...
 stage_clicks AS (
  SELECT uuid,
   ...
   WHERE DATE(clckstrm_event_ts) BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 91 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date)
...
aggregate_clicks AS (
  SELECT ban,
         ...
         -- r90d
         ----------------------------------------------------------------------------------------------------
         COUNT(DISTINCT CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN uuid ELSE NULL END) AS uuid_cnt_r90d,
         ARRAY_AGG(DISTINCT CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN uuid ELSE NULL END IGNORE NULLS) AS uuid_array_r90d,
         COUNT(CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN clckstrm_event_id               ELSE NULL END) AS tot_click_cnt_r90d,
         -- wln
         SUM(CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN wln_ind                           ELSE 0 END) AS wln_tot_cnt_r90d,
         SUM(CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN wln_hsic_ind                      ELSE 0 END) AS wln_hsic_cnt_r90d,
         SUM(CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN wln_tv_ind                        ELSE 0 END) AS wln_tv_cnt_r90d,
         SUM(CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN wln_streaming_ind                 ELSE 0 END) AS wln_streaming_cnt_r90d,
         SUM(CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN wln_security_ind                  ELSE 0 END) AS wln_security_cnt_r90d,
         SUM(CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN wln_smarthome_security_ind        ELSE 0 END) AS wln_smarthome_security_cnt_r90d,
         SUM(CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN wln_online_security_ind           ELSE 0 END) AS wln_online_security_cnt_r90d,
         SUM(CASE WHEN clckstrm_event_dt BETWEEN DATE_SUB((SELECT max_date FROM cte_max_prod_instnc_date), INTERVAL 89 DAY) AND (SELECT max_date FROM cte_max_prod_instnc_date) THEN wln_smartwear_security_ind        ELSE 0 END) AS wln_smartwear_security_cnt_r90d,
  ...
  SELECT ban,
        ...
        --  uuid_cnt_r90d,
        --  uuid_array_r90d,
         sum(tot_click_cnt_r90d) as tot_click_cnt_r90d,
         -- wln
         sum(wln_tot_cnt_r90d) as wln_tot_cnt_r90d,
         sum(wln_hsic_cnt_r90d) as wln_hsic_cnt_r90d,
         sum(wln_tv_cnt_r90d) as wln_tv_cnt_r90d,
         sum(wln_streaming_cnt_r90d) as wln_streaming_cnt_r90d,
         sum(wln_security_cnt_r90d) as wln_security_cnt_r90d,
         sum(wln_smarthome_security_cnt_r90d) as wln_smarthome_security_cnt_r90d,
         sum(wln_online_security_cnt_r90d) as wln_online_security_cnt_r90d,
         sum(wln_smartwear_security_cnt_r90d) as wln_smartwear_security_cnt_r90d,
