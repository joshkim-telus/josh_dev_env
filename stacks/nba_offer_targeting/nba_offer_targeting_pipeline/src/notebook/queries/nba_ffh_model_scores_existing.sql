...
-- This stored procedure pulls the latest propensity scores for various product acquisitions for existing customers. 
...
BEGIN
	...
	from 
		...
		select lpds_id, cust_id,
		'tos_c' as reco,
		case when cs_tos_ultimate_decile = 10 then (0.5-cs_tos_ultimate_score)*1.3
			 when cs_tos_ultimate_decile = 9 then (0.5-cs_tos_ultimate_score)*1.2
			 when cs_tos_ultimate_decile = 8 then (0.5-cs_tos_ultimate_score)*1.1
			 when cs_tos_ultimate_decile = 3 then (0.5-cs_tos_ultimate_score)/1.1
			 when cs_tos_ultimate_decile = 2 then (0.5-cs_tos_ultimate_score)/1.2
			 when cs_tos_ultimate_decile = 1 then (0.5-cs_tos_ultimate_score)/1.3
		else (0.5-cs_tos_ultimate_score)
		end
		as score
		from `cdo-dse-workspace-np-45d0d5.featurestore.nba_ecsus_scored_combined`
		where cs_tos_ultimate_score is not null
		...
		, bacct_num
		, lpds_id
		, row_number() over(partition by cust_id, bacct_num order by lpds_id desc) as row_num
		...
		on a.customer_id = b.cust_id
		and a.bus_bacct_num = b.bacct_num
		where predict_model_id = "5040"
		and create_dt = (select max(create_dt) from `bi-srv-mobilityds-pr-80a48d.ucar_ingestion.bq_product_instance_model_score` where predict_model_id = "5040")
		)
	)
