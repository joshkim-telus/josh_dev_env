...
CREATE OR REPLACE PROCEDURE nba_offer_targeting.bq_sp_irpc_digital_1p_base()

-- This stored procedure pulls the list of customers at cust_id + ban + lpds_id level who are eligible for Digital 1P IRPC Offers
...
BEGIN
	...
	CREATE OR REPLACE TABLE nba_offer_targeting.bq_irpc_digital_1p_base AS
	...
	WITH digital_base1 AS 
		...
		c.hsic_tot_charges_5M AS charges_5M, 
		c.bill_month_1M, 
		c.bill_month_2M, 
		c.bill_month_3M, 
		c.bill_month_4M, 
	...
	SELECT 	cust_id,
			...
			rpp_hsia_end_dt,rpp_ttv_end_dt,
		...
		SELECT  cust_id,
				...
				CASE 	WHEN DATE_DIFF(DATE(rpp_hsia_end_dt), CURRENT_DATE(), DAY) >= 0 THEN charges_30d
						...
						CASE 	WHEN TRIM(bill_month) = TRIM(bill_month_1M) AND charges_30d IS NOT NULL THEN charges_30d 
								...
								WHEN TRIM(bill_month) = TRIM(bill_month_5M) AND charges_5M IS NOT NULL THEN charges_5M 
								END 
	...
	AND NOT (rpp_hsia_end_dt IS NOT NULL AND DATE_DIFF(rpp_hsia_end_dt, rpp_ttv_end_dt, DAY) = 0)
