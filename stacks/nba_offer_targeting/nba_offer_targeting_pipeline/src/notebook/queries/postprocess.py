...
save_data_path = digital_1p_base_postprocess # f'gs://{FILE_BUCKET}/{STACK_NAME}/{IRPC_DIGITAL_1P_BASE}_postprocess.csv'
...
irpc_reco1 = df[df['promo_seg1'] !='']
new_1p_df1 = irpc_reco1['promo_seg1'].apply(lambda x: dict_offer_details.get(x, ['', '']))
promo_seg1 = pd.Series([item[0] for item in new_1p_df1])
offer_code1 = pd.Series([item[1] for item in new_1p_df1])
irpc_reco1['promo_seg'] = promo_seg1
irpc_reco1['offer_code'] = offer_code1
irpc_reco1['Category'] = 'Digital Renewal'
irpc_reco1['Subcategory'] = 'Internet'
...
irpc_reco2 = df[ df['promo_seg2'] !='']
new_1p_df2 = irpc_reco2['promo_seg2'].apply(lambda x: dict_offer_details.get(x, ['', '']))
promo_seg2 = pd.Series([item[0] for item in new_1p_df2])
offer_code2 = pd.Series([item[1] for item in new_1p_df2])
irpc_reco1['promo_seg'] = promo_seg2
irpc_reco1['offer_code'] = offer_code2
irpc_reco2['Category'] = 'Digital Renewal'
irpc_reco2['Subcategory'] = 'Internet'
...
irpc_reco3 = df[df['promo_seg3'] !='']
new_1p_df3 = irpc_reco3['promo_seg3'].apply(lambda x: dict_offer_details.get(x, ['', '']))
promo_seg3 = pd.Series([item[0] for item in new_1p_df3])
offer_code3 = pd.Series([item[1] for item in new_1p_df3])
irpc_reco1['promo_seg'] = promo_seg3
irpc_reco1['offer_code'] = offer_code3
irpc_reco3['Category'] = 'Digital Renewal'
irpc_reco3['Subcategory'] = 'Internet'
