...
CREATE OR REPLACE TABLE FUNCTION shs_churn.shs_churn_label0_tf(_from_dt DATE, _to_dt DATE)
...
AS 
  ...
  label_0_rand AS (
    ...
    , mthly_prod_instnc_snpsht_ts as snapshot_dt -- 1st of each month
    , rand() as rand_num
  ...
  , 1001 AS ban_src_id
  , DATE(snapshot_dt) AS _ref_dt -- 1st of each month
  , 0 AS target_ind
  FROM label_0_ref_dt
