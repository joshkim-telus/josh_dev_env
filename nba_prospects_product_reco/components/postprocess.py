...
def postprocess(
   ...
   # load data from bucket
    df_scores = pd.read_csv(f'gs://{pipeline_bucket}/data/scores.csv')
    ...
    temp_table_name = create_temp_table(
        project_id, dataset_id, table_id, df_scores
    )

    # insert data from temp into main table
    current_part_dt = str(df_scores['part_dt'].max())
    insert_from_temp_table(
        project_id, dataset_id, table_id, temp_table_name, current_part_dt,
