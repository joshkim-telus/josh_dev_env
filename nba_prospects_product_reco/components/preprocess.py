...
def preprocess(
    ...
    sql = (pth_queries / 'load_train_data.sql').read_text().format(
        ...
        target_column=target_column, customer_ids=str_customer_ids,
    ...
    # process features
    df_processed = process_hs_features(
        df, d_model_config, training_mode=True, target_name=target_column
    )
    print(f"Training dataset processed df.shape {df_processed.shape}")

    # save data to pipeline bucket
    df_processed.to_csv(
        f'gs://{pipeline_bucket}/data/training_dataset.csv', index=False
    )
    print(f'Training data saved into {pipeline_bucket}')